...
- 1개 PR 당 약 1~2초 정도 소용되어, 최대 모집 수강생이 20명인 것을 고려하면, 모든 수강생의 PR을 Merge 시키는데에는 최대 에상 소요시간은 대략 40초 정도였습니다.
...
2. 다음으로, 단건 Merge API를 Javascript의 Promise API를 활용하면, 좋을 것 같다고 생각했습니다.
- 왜냐하면, 
- `Promise.all`과 `Promise.allsellted` 중에 고민이었습니다.
- `Promise.all`의 경우, 1개의 Promise가 실패하면, 모든 Promise를 실패로 처리하는 방식이었고,
- `Promise.allsellted`의 경우에는, 부분 성공이 가능한 방식이었습니다.
- 수강생의 PR이 서로 관련이 없기 때문에, 1개가 실패하더라도, 다른 것들은 성공시키는 것이 좋을 거 같다고 생각했습니다.
- 따라서, `Promise.allsellted`로 구현하기로 했습니다.

#### ✨ [결과]
- 결과적으로, PR 일괄 Merge 방식을 비동기 방식으로 변경했기 때문에, 수강생이 늘어나더라도, 총 Merge 소요시간은 최대 10초대를 넘지 않게 되었습니다.
- 이로 인해, 4배 성능 향상
